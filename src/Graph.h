...
typedef struct Graph
{
	...
	int number;				// Количество узлов в графе
} Graph;

typedef struct Node
{
	struct Node *next;		// Следующий по индексу узелу
	struct Node *parent;	// Предыдущий по индексу узелу
	struct HashT *Contact;	// Список связей
	int index;				// Номер узла
	int color;				// Цвет узла
	int number;				// Количество связей
...
typedef struct HashT
{
	...
	struct HashT *next;		// Следующий связный узел
...
Graph *DeleteNodeSave(Graph *graph, Node *node);		// Удаление узла из графа
